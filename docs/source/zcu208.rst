...
* In addition to MATLAB, the following programs and add-ons:
    ...
    * For MathWorks R2024a or newer tools you must use v3.1.1 or newer of RFSoC Explorer
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
[For R2024a or newer ONLY] required MathWorks and 3rd party free software
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
R2024a or newer, only requires the HDL Coder Support Package for Xilinx FPGA and SoC
Devices
...
* `Click to download the Avnet HDL Coder for ZCU208 zip file
  <https://github.com/AvnetDev/hdlcoder-zcu208-zip/releases/download/v1.0.5/zcu208-hdlcoder.zip>`_
...
.. error::
  ...
  R2023b, r2024a and newer and now conditionally adds the correct plugin version.  
  The install script would interfere with this.  The installhdl208.m script is left 
  behind for users that are using older matlab and RFSoC Explorer versions only.
